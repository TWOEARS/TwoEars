...
sig_l = TimeFrequencySignal.construct(sampleRate,bufferSize_s_matlab,'innerhaircell', ihcName, cfHz,'left');
sig_r = TimeFrequencySignal.construct(sampleRate,bufferSize_s_matlab,'innerhaircell', ihcName,cfHz,'right');

lostFrames = 0;

while( 1 )
    ihcOut = rosAFE.ihcPort(ihcName);
    gotFrames = ihcOut.ihcPort.lastFrameIndex - exLastFrameIndex;
    exLastFrameIndex = ihcOut.ihcPort.lastFrameIndex;
    if ( gotFrames > ihcOut.ihcPort.framesOnPort )
        lostFrames = gotFrames - ihcOut.ihcPort.framesOnPort;
        gotFrames = gotFrames - lostFrames;
        disp(strcat('Lost Frames : ', int2str(lostFrames)));
    end

    [ chunkLeft chunkRight ] = adaptTFS( ihcOut.ihcPort.framesOnPort, ...
                                         ihcOut.ihcPort.numberOfChannels, ...
                                         ihcOut.ihcPort.left, ...
                                         ihcOut.ihcPort.right, ...
                                         true );
