...
addpath C:\Dropbox\AMSLearning\Matlab\Tools

addpath C:\Dropbox\AMSLearning\Matlab\latexfigure
...
ndat1 = 200; ndat2 = 200; ndata = ndat1+ndat2;
gmix.centres =  [0.3 0.3; 0.7 0.7]; 
gmix.covars  = [0.01 0.05];
x = gmmsamp(gmix, ndata);

h = figure;
hd = plot(x(:, 1), x(:, 2), '.b', 'markersize', 10);
...
xlabel('1st cepstral coefficient')
ylabel('2nd cepstral coefficient')

% fig2LaTeX('GMM_0',gcf,151);
% close all;
...
xvals = mix.centres(:, 1)*ones(1,ncirc) + sqrt(mix.covars')*xs;
yvals = mix.centres(:, 2)*ones(1,ncirc) + sqrt(mix.covars')*ys;
hc(1)=line(xvals(1,:), yvals(1,:), 'color', 'r','linewidth',2);
hc(2)=line(xvals(2,:), yvals(2,:), 'color', 'b','linewidth',2);
figure(h);
disp(' ')
disp('Press any key to continue')
% pause; clc;
...
%   set(ht, 'string', 'M-step');
  ...
  xvals = mix.centres(:, 1)*ones(1,ncirc) + sqrt(mix.covars')*xs;
  yvals = mix.centres(:, 2)*ones(1,ncirc) + sqrt(mix.covars')*ys;
  hc(1)=line(xvals(1,:), yvals(1,:), 'color', 'k','linewidth',3);
  hc(2)=line(xvals(2,:), yvals(2,:), 'color', 'k','linewidth',3);
%   pause(1)

xlabel('1st cepstral coefficient')
ylabel('2nd cepstral coefficient')

    fig2LaTeX(['GMM_',num2str(n)],gcf,151);
%     close all;
end
