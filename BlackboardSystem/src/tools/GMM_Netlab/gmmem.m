...
if options(5) >= 1
  ...
  check_covars = options(5);	% Ensure that covariances don't collapse
...
for n = 1:niters
  ...
  case 'diag'
    ...
    elseif check_covars == 2
        for j = 1:mix.ncentres
            if ~isPDS(mix.covars(:,:,j))
                % Set weight to zero
                mix.priors(jj) = 0;
            end
        end
  ...
  case 'full'
    ...
    elseif check_covars == 2
        for j = 1:mix.ncentres
            if ~isPDS(mix.covars(:,:,j))
                % Set weight and variance to zero 
                mix.priors(j) = 0;
                mix.covars(:,:,j) = eye(mix.nin);
            end
        end
        % Normalise priors
        mix.priors = mix.priors ./ sum(mix.priors);
